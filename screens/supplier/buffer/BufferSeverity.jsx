...
const BufferSeverity = () => {
  ...
  const { data, isLoading, isError, isFetching } = useBufferQuery(params);
  ...
  const onClickHandler = async () => {
    try {
      const res = await archiveBuffer(bufferData).unwrap();
  ...
  return (
    <Stack mt={3}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} buffer?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this buffer?`}
