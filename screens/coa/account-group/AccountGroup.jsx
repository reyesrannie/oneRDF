...
const AccountGroup = () => {
  ...
  const { data, isLoading, isError, isFetching } = useAccountGroupQuery(params);
  const isTablet = useMediaQuery("(min-width:768px)");

  const archive = useSelector((state) => state.prompt.archive);
  const accountGroupData = useSelector((state) => state.modal.accountGroupData);
  ...
  const [importAccountGroup, { isLoading: loadingImport }] =
    useImportAccountGroupMutation();

  const onClickHandler = async () => {
    try {
      await archiveAccountGroup(accountGroupData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importAccountGroup(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      >
        ...
        {isLoading || isFetching ? (
          ...
          <CardList
            ...
            sub={"code"}
            open={(e, i) => {
              dispatch(setAccountGroupData(i));
              setAnchorEl({
                mouseX: e.clientX,
                mouseY: e.clientY,
              });
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} group?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this group?`}
