...
const AccountCharging = () => {
  ...
  const { data, isLoading, isError, isFetching } = useChargeQuery(params);
  ...
  const [archiveCharge, { isLoading: loadingArchive }] =
    useArchiveChargeMutation();

  const [importCharge, { isLoading: loadingImport }] =
    useImportChargeMutation();

  const onClickHandler = async () => {
    try {
      await archiveCharge(chargeData).unwrap();
      enqueueSnackbar(
  ...
  return (
    <Stack mt={3}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} charging?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this charging?`}
...
export default AccountCharging;
