...
const FinancialStatement = () => {
  ...
  const [archiveFinancialStatement, { isLoading: loadingArchive }] =
    useArchiveFinancialStatementMutation();

  const [importFinancialStatement, { isLoading: loadingImport }] =
    useImportFinancialStatementMutation();

  const onClickHandler = async () => {
    try {
      await archiveFinancialStatement(financialStatementData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importFinancialStatement(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      <MenuPopper
        ...
        update={() => {
          setAnchorEl(null);
          dispatch(setFinancialStatement(true));
        }}
        archive={() => {
          setAnchorEl(null);
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} financial statement?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this financial statement?`}
