...
} from "../../../services/server/api/allocationAPI";
...
const Allocation = () => {
  ...
  const allocationData = useSelector((state) => state.modal.allocationData);
  const importData = useSelector((state) => state.modal.importData);

  const importHeader = [{ name: "name", value: "Name" }];

  const [archiveAllocation, { isLoading: loadingArchive }] =
    useArchiveAllocationMutation();

  const [importAllocation, { isLoading: loadingImport }] =
    useImportAllocationMutation();

  const onClickHandler = async () => {
    try {
      await archiveAllocation(allocationData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importAllocation(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} allocation?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this allocation?`}
