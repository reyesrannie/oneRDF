...
const BusinessUnit = () => {
  ...
  const [archiveBusinessUnit, { isLoading: loadingArchive }] =
    useArchiveBusinessUniMutation();

  const [importBusinessUnit, { isLoading: loadingImport }] =
    useImportBusinessUnitMutation();

  const onClickHandler = async () => {
    try {
      await archiveBusinessUnit(businessUnitData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importBusinessUnit(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} business unit?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this business unit?`}
