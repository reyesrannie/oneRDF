...
const Department = () => {
  ...
  const { data, isLoading, isError, isFetching } = useDepartmentQuery(params);
  ...
  const [archiveDepartment, { isLoading: loadingArchive }] =
    useArchiveDeparmentMutation();
  ...
  const onClickHandler = async () => {
    try {
      await archiveDepartment(departmentData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importDepartment(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      >
        ...
        {isLoading || isFetching ? (
          ...
          <CardList
            ...
            title={"name"}
            sub={"code"}
            tagged={(item) => item.business_unit?.name}
            open={(e, i) => {
              dispatch(setDepartmentData(i));
              setAnchorEl({
                mouseX: e.clientX,
                mouseY: e.clientY,
              });
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} department?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this department?`}
