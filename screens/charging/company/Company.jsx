...
import { icnHeader } from "../../../services/constant/headers";
...
const Company = () => {
  ...
  const [archiveCompany, { isLoading: loadingArchive }] =
    useArchiveCompanyMutation();

  const onClickHandler = async () => {
    try {
      await archiveCompany(companyData).unwrap();
      enqueueSnackbar(
  ...
  return (
    <Stack mt={3}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} company?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this company?`}
