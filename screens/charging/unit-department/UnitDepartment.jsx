...
import DepartmentUnitModal from "../../../components/modal/DepartmentUnitModal";
...
const UnitDepartment = () => {
  ...
  const [archiveDepartmentUnit, { isLoading: loadingArchive }] =
    useArchiveDeparmentUnitMutation();

  const [importDepartmentUnit, { isLoading: loadingImport }] =
    useImportDepartmentUnitMutation();

  const onClickHandler = async () => {
    try {
      await archiveDepartmentUnit(departmentUnitData).unwrap();
      enqueueSnackbar(
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importDepartmentUnit(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Stack mt={3}>
      ...
      <MenuOptions
        ...
        addOption={() => {
          dispatch(setDepartmentUnit(true));
          setAnchorE2(null);
        }}
        importOption={() => {
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} unit?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this unit?`}
