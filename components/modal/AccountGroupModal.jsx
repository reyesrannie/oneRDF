...
const AccountGroupModal = () => {
  ...
  const open = useSelector((state) => state.modal.accountGroup);
  ...
  const [addAccountGroup, { isLoading: loadingAdd }] =
    useAddAccountGroupMutation();
  const [updateAccountGroup, { isLoading: loadingUpdate }] =
    useUpdateAccountGroupMutation();
  ...
  const submitHandler = async (submitData) => {
    const payload = {
      ...submitData,
      id: accountGroupData?.id || undefined,
    };

    try {
      const res =
        accountGroupData !== null
          ? await updateAccountGroup(payload).unwrap()
          : await addAccountGroup(payload).unwrap();
