...
const FinancialStatementModal = () => {
  ...
  const open = useSelector((state) => state.modal.financialStatement);
  ...
  const [addFinancialStatement, { isLoading: loadingAdd }] =
    useAddFinancialStatementMutation();
  const [updateFinancialStatement, { isLoading: loadingUpdate }] =
    useUpdateFinancialStatementMutation();
  ...
  const submitHandler = async (submitData) => {
    const payload = {
      ...submitData,
      id: financialStatementData?.id || undefined,
    };

    try {
      const res =
        financialStatementData !== null
          ? await updateFinancialStatement(payload).unwrap()
          : await addFinancialStatement(payload).unwrap();
  ...
  const mapAccountTitle = () => {
    if (hasRun) return;
    const newData = {
      name: financialStatementData?.name || "",
    };
  ...
  useEffect(() => {
    if (financialStatementData !== null) {
      mapAccountTitle();
    } else {
      reset();
    }
