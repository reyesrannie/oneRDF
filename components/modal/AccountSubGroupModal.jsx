...
const AccountSubGroupModal = () => {
  const dispatch = useDispatch();
  const { enqueueSnackbar } = useSnackbar();
  const open = useSelector((state) => state.modal.accountSubGroup);
  ...
  const [addAccountSubGroup, { isLoading: loadingAdd }] =
    useAddAccountSubGroupMutation();
  const [updateAccountSubGroup, { isLoading: loadingUpdate }] =
    useUpdateAccountSubGroupMutation();
  ...
  const submitHandler = async (submitData) => {
    const payload = {
      ...submitData,
      id: accountSubGroupData?.id || undefined,
    };

    try {
      const res =
        accountSubGroupData !== null
          ? await updateAccountSubGroup(payload).unwrap()
          : await addAccountSubGroup(payload).unwrap();
...
export default AccountSubGroupModal;
