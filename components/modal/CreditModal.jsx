...
const CreditModal = () => {
  ...
  const open = useSelector((state) => state.modal.credit);
  const creditData = useSelector((state) => state.modal.creditData);
  const hasRun = useSelector((state) => state.modal.hasRun);

  const isTablet = useMediaQuery("(min-width:768px)");

  const [addCredit, { isLoading: loadingAdd }] = useAddCreditMutation();
  const [updateCredit, { isLoading: loadingUpdate }] =
    useUpdateCreditMutation();
    ...
    try {
      const res =
        creditData !== null
          ...
          : await addCredit(payload).unwrap();
