...
const NormalBalanceModal = () => {
  const dispatch = useDispatch();
  const { enqueueSnackbar } = useSnackbar();
  const open = useSelector((state) => state.modal.normalBalance);
  ...
  const [addNormalBalance, { isLoading: loadingAdd }] =
    useAddNormalBalanceMutation();
  const [updateNormalBalance, { isLoading: loadingUpdate }] =
    useUpdateNormalBalanceMutation();
  ...
  const submitHandler = async (submitData) => {
    const payload = {
      ...submitData,
      id: normalBalanceData?.id || undefined,
    };

    try {
      const res =
        normalBalanceData !== null
          ? await updateNormalBalance(payload).unwrap()
          : await addNormalBalance(payload).unwrap();
...
export default NormalBalanceModal;
